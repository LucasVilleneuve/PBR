...
public:
	...
	{
		subroutineMap[uniformName] = glGetSubroutineIndex(programId,
			shadertype, uniformName.c_str());

		// Check to ensure that the shader contains a subroutine with this name
		if (subroutineMap[uniformName] == -1)
		{
			std::cout << "Could not add subroutine: " << uniformName << " - location returned -1." << std::endl;
		}
		else // Valid subroutine location? Inform user if we're in debug mode.
		{
			if (DEBUG)
			{
				std::cout << "Subroutine " << uniformName << " bound to location: " << uniformMap[uniformName] << std::endl;
			}
		}
	...
	{
		// Try to find the named subroutine
		auto iter = subroutineMap.find(uniformName);

		// Found it? Great - pass it back! Didn't find it? Alert user and halt.
		if (iter == subroutineMap.end())
		{
